...
new #[Layout('layouts.admin-app')] #[Title('Edit Subscription')] class extends Component {
    public ToolSubscription $toolSubscription;
    ...
    public function mount(ToolSubscription $toolSubscription): void
    {
        $this->toolSubscription = $toolSubscription;

        $this->user_id = (string) $toolSubscription->user_id;
        $this->tool_category_id = (string) $toolSubscription->tool_category_id;
        $this->tool_plan_id = (string) $toolSubscription->tool_plan_id;
        $this->billing_cycle = $toolSubscription->billing_cycle;
        $this->amount = (float) $toolSubscription->amount;
        $this->status = $toolSubscription->status;
        $this->starts_at = $toolSubscription->starts_at?->format('Y-m-d\TH:i');
        $this->expires_at = $toolSubscription->expires_at?->format('Y-m-d\TH:i');

        $this->availablePlans = ToolPlan::query()
            ->where('tool_category_id', $toolSubscription->tool_category_id)
...
<div class="mx-auto space-y-stack-lg">
    <div>
        ...
        <h2 class="font-h1 text-h1 text-on-surface">Edit Subscription</h2>
        <p class="mt-1 text-body-md text-on-surface-variant">
            Update subscription details for {{ $toolSubscription->user?->name ?? 'user' }}.
        </p>
    </div>

    <div class="rounded-xl border border-slate-200 bg-white p-stack-lg shadow-sm">
        <form wire:submit.prevent="update" class="space-y-stack-md">
